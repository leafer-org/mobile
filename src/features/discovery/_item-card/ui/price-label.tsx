...
export function PriceLabel({ price }: { price: ItemListView['price'] }) {
  ...
  if (options.length === 1 && freeOption) {
    return <Text className="text-base font-bold text-stone-900 dark:text-white">Бесплатно</Text>;
  }
  ...
  if (options.length === 1) {
    ...
    if (opt.strategy === 'subscription') {
      return <Text className="text-base font-bold text-stone-900 dark:text-white">{formatPrice(opt.price!)} / мес</Text>;
    }
    return <Text className="text-base font-bold text-stone-900 dark:text-white">{formatPrice(opt.price!)}</Text>;
  }

  const suffix = hasSub ? ' / мес' : '';
  return <Text className="text-base font-bold text-stone-900 dark:text-white">от {formatPrice(minPrice)}{suffix}</Text>;
}
