...
import type { OrganizationContact, OrganizationProfile } from '@/kernel/organization-profile';
...
export function OrganizationContacts({ profile }: { profile: OrganizationProfile }) {
  const isDark = useColorScheme() === 'dark';
  const iconColor = isDark ? '#a8a29e' : '#78716c';

  if (profile.contacts.length === 0) return null;
